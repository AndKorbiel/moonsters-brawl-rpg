...
const damageMath = (calculateDamage, tempOpponentStats) => {
  let defensePoints = tempOpponentStats.stats.filter(
    (el) => el.name === 'defense',
  )[0].value;
  let lifePoints = tempOpponentStats.stats.filter((el) => el.name === 'life')[0]
    .value;
...
export const calculateFightStats = (state) => {
  ...
  let currentAttackPoints = currentPlayer.stats.filter(
    (el) => el.name === 'attack',
  )[0].value;
  let calculateDamage = random(
